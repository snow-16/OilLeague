...
public class SpinnerDrawer : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        this.ObserveEveryValueChanged(_ => SceneProcessor.State).Where(state => state == SceneState.Exist).Subscribe(_ =>
            {
                this.ObserveEveryValueChanged(_ => _spinnerInstanceData.State).TakeUntil(Observable.EveryUpdate().Where(_ => SceneProcessor.State != SceneState.Exist)).Subscribe(state => ChangeImage(state)).AddTo(this);
            }
        );
    }
